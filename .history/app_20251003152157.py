...
class PathItem(ctk.CTkFrame):
    ...
    def __init__(self, parent, path, on_delete=None, **kwargs):
        ...
        self.configure(fg_color="#F5F5F5", corner_radius=6, height=40)
        ...
        # Path text
        # Extract drive letter and path for display
        if ":" in path:
            parts = path.split("\\", 1)
            if len(parts) > 1:
                display_path = f"{parts[1]}: {parts[0]}\\"
            else:
                display_path = path
        else:
            display_path = path
        ...
        self.path_label.grid(row=0, column=1, padx=(0, 10), pady=10, sticky="ew")
    ...
    def select_session(self, session_data):
        ...
        try:
            ...
            for i, window in enumerate(windows):
                ...
                if path:
                    path_item = PathItem(self.paths_frame, path)
