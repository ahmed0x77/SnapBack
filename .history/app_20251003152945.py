...
class PathItem(ctk.CTkFrame):
    ...
    def __init__(self, parent, path, on_delete=None, **kwargs):
        ...
        # Delete button (if callback provided) - hidden by default
        ...
        if on_delete:
            ...
            # Don't grid it yet - will show on hover
            
            # Bind hover events to show/hide delete button
...
class SnapBackApp(ctk.CTk):
    ...
    def create_right_panel(self):
        ...
        self.paths_frame.grid(row=1, column=0, sticky="nsew", padx=30, pady=(20, 30))
    ...
    def delete_path_from_session(self, path_to_delete, session_filepath):
        """Delete a specific path from a session."""
        try:
            # Confirm deletion
            ...
            updated_windows = [w for w in windows if w.get("path") != path_to_delete]
            
            # If all paths removed, delete the entire session
            if not updated_windows:
                messagebox.showinfo(
                    "Session Empty",
                    "This was the last folder in the session. The session will be deleted."
                )
                self.session_manager.delete_session(session_filepath)
                ...
                if self.selected_session:
                    # Reload session data with updated info
                    updated_session_data = self.selected_session.copy()
                    updated_session_data["window_count"] = len(set(w["path"] for w in updated_windows))
                    updated_session_data["tab_count"] = len(updated_windows)
